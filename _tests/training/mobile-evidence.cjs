...
const files=["app.js","index.html","assets/phase3-training-engine.js","assets/training-workout-ui.js","assets/training-workout.css","assets/vendor/lucide-clock.svg"];
...
if(process.argv[2]==="pre"){
 const args=["--test","--test-reporter=tap","_tests/training/mobile.test.cjs","_tests/training/model.test.cjs","_tests/training/preservation.test.cjs","assets/phase6d-theme-authority.test.cjs"];
 ...
 git("diff","--check");
 const result={checked_at:new Date().toISOString(),head:git("rev-parse","HEAD"),command:"node "+args.join(" "),tests:count,passed:count,failed:0,output,completion,
  syntax:["app.js","assets/phase3-training-engine.js","assets/training-workout-ui.js"],source_normalized_lf_hashes:sourceHashes()};
 fs.writeFileSync(path.join(root,"supabase/.temp/training-mobile-unit.json"),JSON.stringify(result,null,2)+"\n");
 console.log(JSON.stringify({tests:count,completion:completion.pass_count,pass:true}));
}else{
 const unit=load("supabase/.temp/training-mobile-unit.json"),local=load("supabase/.temp/training-mobile-browser-local.json"),live=load("supabase/.temp/training-mobile-browser-live.json"),publication=load("supabase/.temp/training-mobile-publication.json");
 ...
 const result={checked_at:new Date().toISOString(),baseline,source_head:head,scope:files,cache:"20260908-training-mobile1",unit,local_summary:summary(local),published_summary:summary(live),local_browser:local,published_browser:live,
  publication,catalog:{count:load("supabase/.temp/training-catalog.json").length,sha256:sha(read("supabase/.temp/training-catalog.json")),basic_groups:context.window.FMZ_WORKOUT_UI.basicGroups},
 ...
 fs.writeFileSync(path.join(root,"docs/TRAINING_MOBILE_EVIDENCE.json"),JSON.stringify(result,null,2)+"\n");
