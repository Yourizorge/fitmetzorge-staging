...
function independent(){
 ...
 '<section><h2>'+t("photo")+'</h2><img class="photo-pair" src="photo-concept.svg" alt="'+esc(t("before")+" / "+t("after"))+'"><div class="photos"><p>A / '+t("before")+'</p><p>B / '+t("after")+'</p></div><p>'+t("photoNote")+'</p>'+btn(t("photoAction"),"photo",{corroborated:true},"",!s.active)+
...
root.addEventListener("click",e=>{
 ...
 if(action==="safety"){result(b.inject(root.querySelector("#safety-select").value));render();return;}
 result(b.command(b.event(next,d)));render();
});
// Read-only diagnostics; no application state, credentials or parent bridge.
window.FMZ8Diagnostics=Object.freeze({view:()=>({route,independent:b.view(),human:a.view()}),copy:()=>clone(FMZ8Copy[lang])});
render();
})();
