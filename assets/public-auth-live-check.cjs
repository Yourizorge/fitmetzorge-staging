...
async function main(){
  const files=[];
  for(const name of ["index.html","fit-met-zorge-logo.png","assets/theme-authority.js","styles.css","config.js","app.js","app.bundle.js","assets/member-ux-consistency.js","assets/phase1-foundation.js","assets/phase5-progress.js","assets/phase6d0-legacy-auth.js","assets/phase6c-private-ai-chat.js","assets/phase6d-owner-settings.js","assets/phase6d-owner-settings.css","assets/phase6d-hotfix2.css","assets/phase6d-analysis-inbox.js","assets/phase3-training-engine.js","assets/youri-ai-avatar-3d-v3-256.webp","assets/vendor/lucide-settings.svg","assets/vendor/lucide-globe.svg","assets/vendor/lucide-x.svg",...fs.readdirSync(path.join(root,"assets/vendor")).filter(name=>/^flag-(nl|gb|de)\.svg$/.test(name)||/^lucide-.*\.svg$/.test(name)&&!["lucide-settings.svg","lucide-globe.svg","lucide-x.svg"].includes(name)).map(name=>"assets/vendor/"+name)]){
    const expected=execFileSync(git,["cat-file","blob",commit+":"+name],{cwd:root,windowsHide:true,maxBuffer:5000000});
    let verified=false;
  ...
  const result={overall_pass:true,commit,cache:"20260907-theme1",files,viewports,databaseMutation:false};
