...
check("4F-B runtime remains present in the owner-corrected 4F-D successor", runtime.includes('PHASE4_SLICE3_VERSION = "20260827-phase4fd-owner-barcode1"'));
check("4F-B runtime is preserved under the owner-corrected 4F-D cache", index.includes("app.js?v=20260827-phase4fd-owner-barcode1") && app.includes("phase4-nutrition-slice3.js?v=20260827-phase4fd-owner-barcode1"));
...
check("OFF selection opens a source-aware detail route", selectFood.includes('food.result_type === "off_branded_food"') && selectFood.includes('openPortal("off"'));
check("OFF selection does not bypass the detail step", selectFood.indexOf('openPortal("off"') < selectFood.indexOf("openEntry(food"));
check("OFF detail hands off explicitly to authoritative logging", offDialog.includes("data-phase4-s3-use-off") && runtime.includes("function openOffEntry("));
check("OFF dialog preserves g/ml isolation", offDialog.includes('text("offUnitOnly"') && runtime.includes("Gram en milliliter worden nooit onderling gelijkgesteld"));
...
check("browser proves OFF authoritative logging handoff", browser.includes("OFF log sends only product identity and no browser nutrients"));
