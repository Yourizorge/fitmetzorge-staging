(() => {
  ...
  const VERSION = "20260903-phase6c-safety-chatux1";
  const LANGUAGES = ["nl", "en", "de"];
  const I18N = {
    nl: {
      nav:"AI Coach",title:"FitMetZorge AI Coach",intro:"Een privegesprek voor heldere, haalbare volgende stappen.",mock:"Vaste simulatie, geen live AI-model.",ready:"Klaar voor je vraag",history:"Gesprekken",options:"Meer opties",privacy:"Privacy en toestemming",
    ...
    en: {
      nav:"AI Coach",title:"FitMetZorge AI Coach",intro:"A private conversation for clear, realistic next steps.",mock:"Fixed simulation, not a live AI model.",ready:"Ready for your question",history:"Conversations",options:"More options",privacy:"Privacy and consent",
  ...
  function installStyles(){
    ...
    const style=document.createElement("style");style.id="phase6c-private-chat-styles";style.textContent=`
      ...
      .p6c-head{display:flex;align-items:center;justify-content:space-between;gap:12px}.p6c-identity{display:flex;align-items:center;gap:11px;min-width:0}.p6c-avatar{width:48px;height:48px;object-fit:cover;border:1px solid rgba(215,178,77,.55);border-radius:8px;background:#111;flex:0 0 auto}.p6c-head h1{font-size:21px;line-height:1.15;margin:0;letter-spacing:0}.p6c-head p{margin:4px 0 0;font-size:13px}.p6c-badges{display:flex;align-items:center;gap:7px;flex-wrap:wrap;justify-content:flex-end}.p6c-ready,.p6c-test{display:inline-flex;align-items:center;gap:6px;min-height:28px;padding:4px 8px;border:1px solid var(--line);border-radius:999px;font-size:12px;white-space:nowrap}.p6c-ready-dot{width:7px;height:7px;border-radius:50%;background:#54b879}.p6c-test{border-color:rgba(215,178,77,.5);background:rgba(215,178,77,.1)}
      ...
      .p6c-composer{position:static;display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:end;gap:8px;padding:10px 12px calc(10px + env(safe-area-inset-bottom));border-top:1px solid var(--line);background:var(--surface);z-index:2}.p6c-composer textarea{width:100%;min-height:48px;max-height:132px;resize:none;overflow-y:auto;padding:12px}.p6c-send{min-width:96px;min-height:48px}.p6c-feedback{grid-column:1/-1;min-height:18px;font-size:13px}.p6c-feedback.error{color:#e59696}.p6c-empty{padding:26px 14px;text-align:center;color:var(--muted)}.p6c-empty h2{color:var(--text);font-size:20px;margin:0 0 7px}.p6c-empty p{margin:0}.p6c-prompts{display:grid;gap:7px;margin:18px auto 0;max-width:520px}.p6c-prompt{min-height:44px;text-align:left;background:transparent!important}.p6c-consent-control{display:flex;justify-content:center}.p6c-consent-control button{min-height:44px;color:var(--muted)}
      ...
      @media(max-width:520px){.p6c-head{align-items:flex-start}.p6c-badges{display:grid;justify-items:end}.p6c-ready{display:none}.p6c-avatar{width:44px;height:44px}.p6c-head h1{font-size:19px}.p6c-chat{min-height:calc(100dvh - 252px)}.p6c-messages{max-height:52dvh}.p6c-composer{grid-template-columns:minmax(0,1fr) 96px;padding-inline:9px}.p6c-send{min-width:96px;padding-inline:8px}}
      @media(max-width:359px){.p6c-head{display:grid}.p6c-badges{justify-items:start}.p6c-toolbar{grid-template-columns:1fr 1fr}.p6c-toolbar button{padding-inline:7px}.p6c-message{max-width:94%}.p6c-composer{grid-template-columns:1fr}.p6c-send{width:100%}}
  ...
  function renderMessages(){return chat.messages.length?chat.messages.map((item)=>{const safety=isSafetyMessage(item);return `<article class="p6c-message ${item.message_role==='user'?'user':'assistant'} ${safety?'safety':''}"><strong>${esc(safety?text("safetyLabel"):item.message_role==='user'?text("you"):text("coach"))}</strong><p>${esc(item.content_text)}</p><time>${esc(time(item.created_at))}</time></article>`;}).join(""):`<div class="p6c-empty"><h2>${esc(text("emptyMessages"))}</h2><p>${esc(text("emptyHint"))}</p><div class="p6c-prompts">${["promptOne","promptTwo","promptThree"].map((key)=>`<button class="secondary-btn p6c-prompt" type="button" data-p6c-prompt="${esc(text(key))}">${esc(text(key))}</button>`).join("")}</div></div>`;}
  function renderChat(){
    ...
    return `<section class="p6c-chat"><header class="p6c-chat-head"><h2>${esc(date(chat.thread.created_at)||text("today"))}</h2><details class="p6c-actions"><summary aria-label="${esc(text("options"))}" title="${esc(text("options"))}">•••</summary><div class="p6c-action-menu"><button class="secondary-btn" type="button" data-p6c-export>${esc(text("export"))}</button><button class="secondary-btn p6c-danger" type="button" data-p6c-delete>${esc(text("delete"))}</button></div></details></header><div class="p6c-messages" data-p6c-messages>${renderMessages()}${processing}</div>${canWrite?`<form class="p6c-composer" data-p6c-form><label class="sr-only" for="p6cMessage">${esc(text("placeholder"))}</label><textarea id="p6cMessage" rows="1" maxlength="4000" enterkeyhint="enter" autocomplete="off" placeholder="${esc(text("placeholder"))}" ${chat.pending?"disabled":""}>${esc(chat.draft)}</textarea><button class="primary-btn p6c-gold p6c-send" type="submit" ${chat.pending||!navigator.onLine?"disabled":""}>${esc(chat.pending?text("sending"):chat.retry?text("retry"):text("send"))}</button><div class="p6c-feedback ${chat.error?"error":""}" aria-live="polite">${esc(chat.error||chat.notice)}</div></form>`:""}</section>`;
  }
