-- Current-state frozen 6A contract, after accepted 6C and authorized 6D integrations.
-- Preserves all 47 assertions. Scopes the eight original policies and six original
-- purpose documents to their original objects; newer 6D objects are tested separately.
-- Two remaining unqualified digest consumers retain extensions in search_path.
-- Other fixed-path functions use qualified helpers after the preceding owner hotfix.
-- No DDL, DML or member RPC.
with
expected_tables(table_schema, table_name, columns) as (
  values
    ...
    ('public','ai_threads',array['id:uuid','user_id:uuid','feature_code:text','locale:text','status:text','retention_state:text','retention_started_at:timestamptz','retention_due_at:timestamptz','content_deleted_at:timestamptz','created_at:timestamptz','updated_at:timestamptz','archived_at:timestamptz','client_request_id:uuid','revision:int8','last_message_sequence:int8']::text[]),
    ('public','ai_messages',array['id:uuid','user_id:uuid','thread_id:uuid','message_role:text','feature_code:text','content_text:text','structured_output:jsonb','schema_version:text','status:text','request_id:uuid','run_id:uuid','created_at:timestamptz','sequence_number:int8']::text[]),
    ...
    ('public','ai_data_lifecycle_requests',array['id:uuid','user_id:uuid','request_type:text','status:text','request_id:uuid','requested_at:timestamptz','completed_at:timestamptz','safe_result_code:text','scope_thread_id:uuid']::text[]),
...
policies as (
  ...
  from pg_policies
  where (schemaname,tablename) in (select table_schema,table_name from expected_tables where table_schema='public')
),
functions as (
...
checks(check_name, pass, detail) as (
  values
    ...
    ('foreign_key_indexes', (select count(*)=8 from indexes where name in ('ai_messages_thread_owner_idx','ai_action_proposals_context_owner_idx','ai_action_decisions_proposal_owner_idx','ai_runs_thread_owner_idx','ai_runs_context_owner_idx','ai_budget_accounts_policy_idx','ai_safety_events_run_idx','ai_audit_events_run_idx')), 'all Phase 6A foreign keys have covering indexes'),
    ...
    ('consent_documents_nl_en_de', (select count(*) = 6 and count(distinct locale)=3 and count(distinct consent_kind)=2 and bool_and(status='active') from ai_private.consent_documents where consent_kind in ('ai_processing','trainer_summary_sharing')), 'six original AI and trainer-summary documents'),
    ...
    ('safe_search_paths', not exists(select 1 from functions where not (
      'search_path=pg_catalog, public, ai_private, pg_temp'=any(config)
      or 'search_path=pg_catalog, extensions, public, ai_private, pg_temp'=any(config)
    )) and (select array_agg(name order by name)=array['fmz_phase6a_get_context_manifest','fmz_phase6a_service_begin_run'] from functions where 'search_path=pg_catalog, extensions, public, ai_private, pg_temp'=any(config)), 'fixed search_path; two exact remaining unqualified digest consumers'),
    ...
    ('no_actions_or_decisions', not exists(select 1 from public.ai_action_proposals) and not exists(select 1 from public.ai_action_decisions), 'accepted private chat may exist; no proposal/action package active'),
    ('member_runs_mock_zero_cost', not exists(select 1 from ai_private.runs where adapter_code <> 'mock' or reserved_cost_micros <> 0 or actual_cost_micros <> 0), 'accepted mock runs may exist; no member provider run or reservation'),
