-- Package 6C owner-freeze regression verifier, 2026-09-04.
-- Derived from the immutable 6A installation verifier; only six accepted 6C
-- columns and two post-install lifecycle expectations differ. No DDL/DML/RPC.
-- Run with the unchanged 6B and both 6C verifiers; not an installation checker.
with
...
checks(check_name, pass, detail) as (
  values
    ...
    )) and (select count(*)=3 from functions where 'search_path=pg_catalog, extensions, public, ai_private, pg_temp'=any(config)), 'fixed search_path; pgcrypto schema only on three digest consumers'),
