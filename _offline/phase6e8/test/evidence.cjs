...
const browser=kind=>{const file="supabase/.temp/phase6e8-browser/"+kind+".json";if(!fs.existsSync(path.join(root,file)))return null;const raw=fs.readFileSync(path.join(root,file)),r=JSON.parse(raw);return {artifact:file,sha256:sha(raw),checks:r.checks.length,passed:r.checks.filter(x=>x.pass).length,layouts:r.layouts.length,static_preloads:r.static_preloads?.length||0,errors:r.errors,screenshots:r.screenshots.map(f=>({artifact:"supabase/.temp/phase6e8-browser/"+path.basename(f),sha256:sha(fs.readFileSync(f))}))};};
