...
try{
 ...
 let currentSession,loseApply=false;
 await page.route(BASE+"/functions/v1/fmz6e10-synthetic",async route=>{
  if(loseApply&&route.request().method()==="POST"&&route.request().postDataJSON()?.action==="apply"){
   loseApply=false;await route.fetch();await route.abort("failed");return;
  }
 ...
 check("signed-out page contains no synthetic plan data",await page.locator("#login").count()===1&&await page.locator(".proposal").count()===0);
 ...
 check("operator creates and activates a fresh managed window",!!windowId);
 ...
 check("v1 created by authenticated trainer",await page.locator(".source").count()===1);
 await click('[data-action="propose"]');
 await login("member");
 await click('[data-action="member_accept"]:not([disabled])');
 check("member cannot act as trainer",await page.locator('[data-action="trainer_approve"]').count()===0);
 await page.reload();await page.locator(".member_accepted").waitFor();await idle();
 check("refresh preserves server member acceptance",await page.locator(".member_accepted").count()===1);
 ...
 await oldJwt.text();check("logout denies still-signed browser JWT", [401,403].includes(oldJwt.status));
 await login("trainer");
 await click('[data-action="new-source"]');
 ...
 await page.locator('#source-form button[type="submit"]').click();await idle();
 check("v2 leaves v1 proposal stale and application unavailable",await page.locator(".source").count()===2&&await page.locator(".warning").count()>0&&await page.locator('[data-action="apply"]:not([disabled])').count()===0);
 await click('[data-action="propose"]');
 await login("member");
 await click('[data-action="member_accept"]:not([disabled])');
 await login("trainer");
 await click('[data-action="trainer_approve"]:not([disabled])');
 check("trainer approval leaves separate apply button",await page.locator(".approved").count()===1&&await page.locator('[data-action="apply"]:not([disabled])').count()===1);
 ...
 try{
  ...
  check("trainer unlinked after approval cannot apply",[401,403].includes((await applyAttempt()).status));
 ...
 try{
  ...
  check("withdrawn consent after approval cannot apply",(await applyAttempt()).status===403);
  await api.sql("update fmz6e10_private.workspaces set consent=true,guard='self_reported' where id="+api.lit(active.workspace));
  check("self-reported recovery is not application clearance",(await applyAttempt()).status===403);
 }finally{await api.sql("update fmz6e10_private.workspaces set consent=true,guard='clear' where id="+api.lit(active.workspace));}
 check("all blocked apply attempts preserve plan version",(await api.raw("member",{op:"read",window:windowId,workspace:active.workspace})).data.active_version===1);
 ...
 check("18 responsive language/theme layouts",report.layouts.length===18);
 await page.locator("#language").selectOption("nl");
 loseApply=true;
 await click('[data-action="apply"]:not([disabled])');
 check("uncertain application offers same-request retry",await page.locator('[data-action="retry"]').count()===1);
 await click('[data-action="retry"]');
 check("separate application creates v2 and disables duplicate action",await page.locator(".applied").count()===1&&await page.locator('[data-action="apply"]:not([disabled])').count()===0);
 await login("member");
 ...
 check("restore creates version 3 with source history retained",await page.locator(".plan").count()===3);
 ...
 for(const action of ["member_reject","trainer_reject","trainer_block"]){
  ...
  check(action+" commits its terminal status",r.status===(action==="trainer_block"?"blocked":"rejected"));
  ...
  check(action+" cannot be applied",denied.status===403&&v.active_version===3);
 }
 ...
 check("W2 displays full configured step and attempted weight without partial apply",blocked.includes("Ingestelde stap: 3 kg")&&blocked.includes("Berekend gewicht: 43 kg")&&await page.locator('[data-action="apply"]:not([disabled])').count()===0);
 await click('[data-action="withdraw"]');
 await click('[data-action="propose"]');
 check("withdrawn source gives explicit server refusal",/synthetic_source_unavailable/.test(await page.locator("#feedback").innerText()));
 await login("b");
 check("B has neither sources nor A actions nor management",await page.locator(".source").count()===0&&await page.locator(".proposal").count()===0&&await page.locator("#management").count()===0);
 await login("trainer");
 await page.locator("#management summary").first().click();
 await click('[data-action="revoke"]:not([disabled])');
 check("revoke removes active workspace on refresh",await page.locator("#route-content").count()===0);
 await click('[data-action="cleanup"]:not([disabled])');
 check("cleanup is visible and disabled after completion",await page.locator(".cleaned").count()>0);
