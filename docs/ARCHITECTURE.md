...
This 6B boundary is live only on staging as migration history `20260902045834` and `youri-ai` v2 with JWT verification. The verifier passes 36/36 and the rollback E2E leaves zero rows/calls/cost. Paid synthetic acceptance awaits server-only OpenAI account credentials; this does not weaken the real-member block.
