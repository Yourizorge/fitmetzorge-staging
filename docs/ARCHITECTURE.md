...
The browser only presents the server result. Its compact mobile chat identifies the persona as `Youri AI` under the unchanged `FitMetZorge AI Coach` product, uses a neutral avatar pending owner photo/approval, and keeps the growing textarea and send action in one horizontal bottom row. History, message bubbles, prompt chips, processing/retry states and secondary export/delete controls add no classifier or domain authority. The path has no provider import or provider call, and cannot select a model or action. It is live on staging as unchanged `youri-ai` v40 with JWT verification; commit `8dfa235`, cache `20260903-phase6c-continued-chat1`, migration history `20260903125150` and bundle SHA-256 `453309b755c53aca1754c8cb529dbb340694a5828e7b049a1c3b40e63530fcf3`. Package 6C is ready for owner retest, but is not owner-accepted or frozen.
