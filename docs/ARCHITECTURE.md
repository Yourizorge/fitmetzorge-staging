...
Slice 4E is locally prepared for migration review. It adds private `nutrition_food_ingestions` audit state with no app or service-role ACL, nullable restrictive ingestion links on `foods` and `food_aliases`, canonical reviewed/verified visibility that requires a ledger link, and explicit preferred-NL-alias uniqueness per normalized term and market. The exact existing `fmz_phase4_search_foods(text,integer,text,uuid)` signature becomes alias-aware without a frontend contract change. It remains `SECURITY INVOKER`, uses bounded indexed candidate branches, deduplicates by food UUID, and reconstructs the last row's rank from `p_after_id` for stable rank-aware keyset pagination. No catalog record, alias, portion, provider call, public ingestion route, or production change is part of the migration.
